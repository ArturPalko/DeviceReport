...
void MainWindow::initListDevice()
{
    ...
    mainRoot=addRootDevClass("Кількість пристроїв: ", -1, roots.size());

    //addCildDev(mainRoot, "",-1,0);
    //mainRoot->setText(0,"Кількість пристроїв: "+QString::number(countOfDevices));
...
void MainWindow::initListDevInfo()
{
    ...
    ui->listwDeviceInfo->item(1, 0)->setText("Клас пристрою");
    ui->listwDeviceInfo->item(2, 0)->setText("Перелічувач");
    ui->listwDeviceInfo->item(3, 0)->setText("Виробник");
    ui->listwDeviceInfo->item(4, 0)->setText("Ім'я об'єкту ФП");
    ui->listwDeviceInfo->item(5, 0)->setText("Служба");
    ui->listwDeviceInfo->item(6, 0)->setText("ID пристрою");
    ui->listwDeviceInfo->item(7, 0)->setText("Шлях INF-файлу");
    ui->listwDeviceInfo->item(8, 0)->setText("Дата INF-файлу");
    ui->listwDeviceInfo->item(9, 0)->setText("Секція INF-файлу");
}

void MainWindow::progressAction()
{
    timer->start(3);
    lbProgress->setText("Оновлення");
    prRefresh->setValue(prRefresh->value()+1);
    if(prRefresh->value()>=100)
    {
        timer->stop();
        prRefresh->reset();
        lbProgress->setText("Виконано");
    }
}
...
void MainWindow::on_actRefresh_triggered()
{
    ...
    {
        QTreeWidgetItem *root=addRootDevClass("Displays adapters", -1, 1);
